...
function getCartTotalAmount() {
  ...
  //   result += SHOPPING_CART[i];
  // }
for(let i = 0; i < cart.length; i++){
  for(let j = 0; j < pro.length; j++){
    if(cart[i].id == pro[j].id){
      result += cart[i].quantity*pro[j].price;
    }
  }
}
 ...
 * the product id already exists in the cart,  and quantity if >=2    :  just decrement its quantity
 *      example :  removeProductToCart(1)    :   [{ id: 1, quantity: 2 }]   ------>   [{ id: 1, quantity: 1 }]
 *
 *  - CASE 2 : if the product id already exists
 * @param {*} productId  the product id to add
 */
